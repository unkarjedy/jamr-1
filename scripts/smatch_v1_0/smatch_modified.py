...
Modified by Naumenko Dmitrii to log progress (also auto-formated code (was bad-formated, bugs occured))
precision/recall/f1
...
def build_arg_parser2():
    ...
    parser.add_option("--vNaum", "--verboseNaum", action='store_true', dest="vNaum", help='Verbose Dmitrii Naumenko output (Default:False)')
...
def compute_pool(test_instance, test_relation1, test_relation2, gold_instance, gold_relation1, gold_relation2,
    ...
    for i in range(0, len_test_inst):
        for j in range(0, len_gold_inst):
            if test_instance[i][0].lower() == gold_instance[j][0].lower() and test_instance[i][2].lower() == \
                    gold_instance[j][2].lower():
    ...
    for i in range(0, len_test_rel1):
        for j in range(0, len_gold_rel1):
            if test_relation1[i][0].lower() == gold_relation1[j][0].lower() and test_relation1[i][2].lower() == \
                    gold_relation1[j][2].lower():
...
def get_fh(test_instance, test_relation1, test_relation2, gold_instance, gold_relation1, gold_relation2, test_label,
           gold_label):
    ...
    (candidate_match, weight_dict) = compute_pool(test_instance, test_relation1, test_relation2, gold_instance,
                                                  gold_relation1, gold_relation2, test_label, gold_label)
    best_match_num = 0
    ...
    for i in range(0, iter_num):
        ...
        # hill-climbing
        (largest_match_num, cur_match) = get_best_gain(start_match, candidate_match, weight_dict, len(gold_instance),
                                                       match_num)
        if verbose:
        ...
        while largest_match_num > match_num:
            match_num = largest_match_num
            (largest_match_num, cur_match) = get_best_gain(cur_match, candidate_match, weight_dict, len(gold_instance),
                                                           match_num)
            if verbose:
                print >> sys.stderr, "Largest match number after the hill-climbing", largest_match_num
...
def print_alignment(match, test_instance, gold_instance, flip=False):
    ...
    for i, m in enumerate(match):
        if m == -1:
            ...
            if not flip:
                result.append(test_instance[i][1] + "(" + test_instance[i][2] + ")" + "-" + gold_instance[m][1] + "(" +
                              gold_instance[m][2] + ")")
            else:
                result.append(gold_instance[m][1] + "(" + gold_instance[m][2] + ")" + "-" + test_instance[i][1] + "(" +
                              test_instance[i][2] + ")")
    return " ".join(result)
...
def main(args):
    ...
    if args.vNaum: verboseNaum = True
    else: verboseNaum = False
    ...
    total_sentences = 0
    with open(args.f[0].name) as file:
        for line in file:
            if line == "\n":
                total_sentences  += 1
    if(verboseNaum):
    ...
    while True:
        if(verboseNaum):
            sys.stdout.write(str(sent_num) + " ")
            sys.stdout.flush()
        if(sent_num >= total_sentences - 1):
            debug = 2 + 2
        ...
        if len(test_inst) < len(gold_inst):
            (best_match, best_match_num) = get_fh(test_inst, test_rel1, test_rel2, gold_inst, gold_rel1, gold_rel2,
                                                  test_label, gold_label)
            if verbose:
            ...
            (best_match, best_match_num) = get_fh(gold_inst, gold_rel1, gold_rel2, test_inst, test_rel1, test_rel2,
