...
class ConceptDecoder(options: m.Map[Symbol, String],
  ...
  def decode(input: Input,
             ...
             calcExtraCost: ExtraCostFunc): DecoderResult = {
    ...
    for (wordId <- sentenceWords.indices) {
      ...
      logger(1, "word = " + sentenceWords(wordId))
      logger(1, "Possible invoked concepts: " + possibleConcepts.map(_.toString).mkString("\n"))
      ...
      for (concept <- possibleConcepts) {
        ...
        if (exeedsSentenceBorders) {
          ...
          // TODO: NAUMENKO
          // TODO: upper comment is weird, you should use < here if you want earlier concepts to have higher priority
