...
class AdagradBasic extends Optimizer[FeatureVectorBasic] {
  def learnParameters(gradient: (Option[Int], Int, FeatureVectorBasic) => (FeatureVectorBasic, Double),
    ...
    val sumSq = FeatureVectorBasic() // G_{i,i}
    var pass = 0
    ...
    while (pass < passes && (pass == 0 || trainingObserver(pass, weights))) {
      ...
      for (t <- Random.shuffle(Range(0, trainingSize).toList)) {
        ...
        objective += score
        if (l2reg != 0.0) {
          // TODO: don't count the unregularized features in the regularizer
          objective += weights.dot(weights) / (2.0 * trainingSize)
