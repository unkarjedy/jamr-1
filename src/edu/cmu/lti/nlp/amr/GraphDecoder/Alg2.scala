...
class Alg2(options: Map[Symbol, String],
  ...
  def decode(): DecoderResult = {
    ...
    def addEdge(node1: Node, index1: Int,
                node2: Node, index2: Int,
                label: String,
                weight: Double,
    ...
    val neighbors: Array[Array[(String, Double)]] = {
      for ((nodes2, index1) <- edgeWeights.zipWithIndex) yield {
        ...
        for ((labelWeights, index2) <- nodes2.zipWithIndex) yield {
          ...
          val (label, weight) = labelWeights.view.zipWithIndex.map(x => {
            (x._1._1, x._1._2 + {
              val (f, v, _) = features.ffLRLabelWithId(node1, node2).head;
              features.weights(f, Some(x._2)) * v.conjoined
            })
          }).maxBy(_._2)
    ...
    //logger(1, graph.printTriples(detail = 1)+"\n")
    if (nodes.size > 0) {
      def validForRootFeatures(node: Node) = {
        !node.concept.startsWith("\"") && !node.concept.matches("[0-9].*")
      }

      if (features.rootFeatureFunctions.nonEmpty && nodes.exists(validForRootFeatures)) {
        graph.root = findRootNode(nodes)
      } else {
        graph.root = nodes(0) //logger(1, "Setting root to "+nodes(0).id)
      }

      feats += features.rootFeatures(graph.root)
      score += features.rootScore(graph.root)

      nodes.foreach(node => node.relations = node.relations.reverse)
      //if (connected) {
