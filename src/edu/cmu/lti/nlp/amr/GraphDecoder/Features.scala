...
class Features(options: mutable.Map[Symbol, String], private var myFeatureNames: List[String], labelSet: Array[String]) {
  ...
  val ffTable = mutable.Map[String, FeatureFunction](
  ...
  // TODO
  def ffTypeBias {
    def conceptType(x: String): String = if (x.matches(".*-[0-9][0-9]")) {
      "E"
    } else {
      "O"
    }
    def labelType(x: String): String = if (x.startsWith(":ARG")) {
      "A"
    } else {
      "O"
    }
  ...
  def ffSelf {
    addFeature("Slf", if (node1.spans(0) == node2.spans(0)) {
      1.0
    } else {
      0.0
    }, 0.0)
  ...
  def ffFragHead {
    ...
    addFeature("C1NotFragHead", if (node1.concept != graph.spans(node1.spans(0)).amrNode.concept) {
      1.0
    } else {
      0.0
    }, 0.0)
    addFeature("C2NotFragHead", if (node2.concept != graph.spans(node2.spans(0)).amrNode.concept) {
      1.0
    } else {
      0.0
    }, 0.0)
  ...
  def ffPosPathUnigramBigramv1 {
    ...
    val pp = "PPv1"
    val direction = if (graph.spans(node1.spans(0)).start < graph.spans(node2.spans(0)).start) {
      "+1"
    } else {
      "-1"
    }
  ...
  def ffDependencyPathv2 {
    val (word1Index, word2Index, path) = (for {w1 <- dependencySpan(node1)
      ...
      (w1, w2, dependencyPath(w1, w2))
    }).minBy(x => x._3._1.size + x._3._2.size)
  ...
  def depPathStrv3(node1: Node, node2: Node, maxpath: Int = 4): String = {
