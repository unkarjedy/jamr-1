...
import edu.cmu.lti.nlp.amr.{Wordnet, max}

import scala.collection.mutable.Map
import scala.util.matching.Regex

/** **************************** Align Words *****************************/
case class  SpansAlignerWrapper2(logger: JAMRLogger) {
    private val log = logger.log _

    /* Contains the list of tules (14? or something like...) */
    def align(sentence: Array[String], graph: Graph) {
        ...
        val namedEntity = new SpanAligner2(sentence, graph) {
            concept = "name"
        ...
        val fuzzyNamedEntity = new SpanAligner2(sentence, graph) {
            concept = "name"
            tabSentence = "\t" + sentence.mkString("\t").toLowerCase + "\t" /*.replaceAll("[^a-zA-Z0-9\t]","")*/
            nodes = node => {
                if (node.children.exists(_._1.matches(":op.*"))) {
        ...
        val namedEntityCollect = new SpanUpdater2(sentence, graph, wordToSpan) {
            concept = "name"
            nodes = node => {
                if (node.children.count(x => x._1.matches(":op.*") && isAligned(x._2, graph)) == node.children.size && node.children.nonEmpty) {
        ...
        val dateEntity = new SpanAligner2(sentence, graph) {
            concept = "date-entity"
            ...
            words = nodes => {
                ...
                val yearStr = year.headOption.getOrElse("")
                val monthStr = month.headOption.getOrElse("")
                val dayStr = day.headOption.getOrElse("")
                ((year ::: month ::: day).permutations.map("\t0*" + _.mkString("\t*0*") + "\t").mkString("|")
                  + "|\t" + to2Digit(yearStr) + to2Digit(monthStr) + to2Digit(dayStr) + "\t").r
            }
        }

        val minusPolarity = new SpanAligner2(sentence, graph) {
            concept = "-"
            tabSentence = "\t" + sentence.mkString("\t").toLowerCase + "\t"
            var word = List("no", "not", "non") // TODO: Add anti, anti- Anti
            nodes = node => {
                if (sentence.exists(x => word.contains(x))) {
                    List(("", node))
                } else {
                    List()
                }
        ...
        val singleConcept = new SpanAligner2(sentence, graph) {
            concept = "[^-].*" // so :polarity - doesn't get matched (:op1 "-" still does though)
            nodes = node => {
                if (alignWord(stemmedSentence, node, wordToSpan).isDefined) {
                    List(("", node))
                } else {
                    List()
                }
            ...
            spans = nodes => {
                val Some(index) = alignWord(stemmedSentence, nodes.head._2, wordToSpan)
                List((index, index + 1))
            }
        }

        val fuzzyConcept = new SpanAligner2(sentence, graph) {
            concept = "[^-].*" // so :polarity - doesn't get matched (:op1 "-" still does though)
            nodes = node => {
        ...
        val US = new ConceptAndChildren(sentence, graph) {
            concept = "name"
            children = Map(":op1" -> "United", ":op2" -> "States")
            words = x => {
                "\tus\t|\tu[.]\t?s[.]\t".r
            }
        }
    ...
    class SpanAligner2(val sentence: Array[String], val graph: Graph) {
        ...
        var spans: (List[(String, Node)]) => List[(Int, Int)] = allNodes => {
            ...
            log(2, "matchList: " + matchList)
            matchList.map(x => matchToIndices(x, tabSentence))
        }

        def getSpans(node: Node): List[Span] = {
            log(2, "Processing node: " + node.concept)
            node match {
                case Node(_, _, c, _, _, _, _, _) if concept.r.unapplySeq(getConcept(c)).isDefined && !node.isAligned(graph) =>
                    log(2, "Matched concept regex: " + concept)
                    log(2, "tabSentence: " + tabSentence)
                    val allNodes = nodes(node)
                    log(2, "allNodes: " + allNodes.map(x => (x._1, getConcept(x._2.concept))).toString)
    ...
    class SpanUpdater2(val sentence: Array[String],
                       val graph: Graph,
        ...
        def update(node: Node) {
            ...
            node match {
                case Node(_, _, c, _, _, _, _, _) if concept.r.unapplySeq(getConcept(c)).isDefined && (!node.isAligned(graph) || !unalignedOnly) => {
                    log(2, "SpanUpdater matched concept regex: " + concept)
                    val allNodes = nodes(node)
                    log(2, "allNodes = " + allNodes.toString)
                    val updateIndices = if (allNodes.nonEmpty) {
                        spanIndex(allNodes)
                    } else {
                        List()
                    }
                    log(2, "updateIndices = " + updateIndices.toString)
                    val wordSpans = if (allNodes.nonEmpty) {
    ...
    class ConceptAndChildren(override val sentence: Array[String],
                            ...
                            ) extends SpanAligner2(sentence, graph) {
        ...
        nodes = node => {
            ...
            if (node.children.count(x => checkMatch(x)) == children.size) {
    ...
    class UnalignedConcept(override val sentence: Array[String],
      ...
      extends SpanUpdater2(sentence, graph, wordToSpan) {
        ...
        nodes = node => {
            if (node.children.exists(_._1.matches(label)) && node.children.count(_._1.matches(label)) <= num) {
                try {
                    List(("", node) :: graph.spans(node.children.filter(_._1.matches(label)).head._2.spans(0)).nodeIds.map(x => ("", graph.getNodeById(x))))
                } catch {
    ...
    class UnalignedChild(override val sentence: Array[String],
      ...
      extends SpanUpdater2(sentence, graph, wordToSpan) {
        ...
        nodes = node => {
            if (node.children.exists(_._1.matches(label)) && node.children.filter(_._1.matches(label)).head._2.spans.isEmpty && sentence.slice(graph.spans(node.spans(0)).start, graph.spans(node.spans(0)).end).mkString("\t").matches(words)) {
                try {
                    val child: Node = node.children.filter(_._1.matches(label)).head._2
        ...
        spanIndex = nodes => nodes.map(x => x.head._2.spans(0)) // 1nd node of each span
        unalignedOnly = false
    }
    ...
    private def toSpan(startEnd: (Int, Int), nodeIds: List[String], sentence: Array[String], graph: Graph, coRef: Boolean): Span = {
        ...
        assert(nodeIds.nonEmpty, "Error: matchToSpan passed nodeIds with zero length")
    ...
    private def matchToIndices(m: Regex.Match, tabSentence: String): (Int, Int) = {
        ...
        (getIndex(tabSentence, m.start), getIndex(tabSentence, m.end) - 1)
    }

    private def getIndex(tabSentence: String, charIndex: Int): Int = {
        log(2, "slice = [" + tabSentence.slice(0, charIndex).toString + "]")
        tabSentence.view.slice(0, charIndex).count(_ == '\t') // views (p.552 stairway book)
    }
    ...
    private def addAllSpans(f: SpanAligner2, graph: Graph, wordToSpan: Array[Option[Int]], addCoRefs: Boolean) {
        def add(node: Node) {
            var added = false
    ...
    private def updateSpans(f: SpanUpdater2, graph: Graph) {
    ...
    def matchLength(string1: String, string2: String): Int = {
        var length = 0
        for (i <- Range(0, Math.min(string1.length, string2.length))) {
